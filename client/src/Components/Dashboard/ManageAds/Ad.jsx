...
export default function Ad(props) {
  ...
  const deleteAd=()=>{
    axios.post("http://localhost:3001/deleteMyAd", {
      id_Ad : props.id_Ad
   }).then(() => {
      console.log('done');
   })
   ...
   const editAd=()=>{
     let title=props.titre.trim();
     for(var s of title) title=title.replace("%","PourCent");
     let description=props.description.trim();
     for(var s of description) description=description.replace("%","PourCent");

      history.push(`/editAd/${props.id_Ad}/${title}/${description}/${props.email}/${props.phone}/${props.ville}`)
   }
  return (
